...
# A discovered contact is "fresh" if the Companion lastmod was updated within
# this window. Mirrors the per-contact binary_sensor freshness threshold
...
class HiveFWStatusCardSensor(CoordinatorEntity, SensorEntity):
    ...
    def native_value(self) -> Any:
        ...
        if key == "dashboard_network_activity":
            now = time.time()
            def seen_at(item):
                return float(item.get("lastmod") or item.get("last_modified") or 0)
            return sum(
...
class MeshCoreDiscoveredSummarySensor(CoordinatorEntity, SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:  # type: ignore[override]
        ...
        for contact in discovered.values():
            lastmod = contact.get("lastmod") or contact.get("last_modified") or 0
            if lastmod and (now - lastmod) < DISCOVERED_FRESH_WINDOW_SECS:
                fresh_count += 1
        ...
        if newest_contact is not None:
            ...
            newest = {
                ...
                "last_modified": newest_contact.get("lastmod") or newest_contact.get("last_modified") or 0,
